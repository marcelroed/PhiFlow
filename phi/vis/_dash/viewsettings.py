...
def build_view_selection(app: DashApp, field_selections: tuple, viewer_group: str):
    ...
    for sel_dim in parse_dim_order(app.config.get('select', [])):
        sel_pane = html.Div(style={'height': '50px', 'width': '100%', 'display': 'flex', 'align-items': 'center',
        ...
        @app.dash.callback(Output(f'{viewer_group}_select_{sel_dim}', 'max'),
                           [STEP_BUTTON, REFRESH_INTERVAL, PAUSE_BUTTON, *field_selections])
        ...
        @app.dash.callback(Output(f'{viewer_group}_select_{sel_dim}', 'disabled'),
                           [STEP_BUTTON, REFRESH_INTERVAL, PAUSE_BUTTON, *field_selections])
        ...
        @app.dash.callback(Output(f'{viewer_group}_select_{sel_dim}', 'marks'),
                           [STEP_BUTTON, REFRESH_INTERVAL, PAUSE_BUTTON, *field_selections])
        def update_dim_disabled(_s, _r, _p, *field_names, dim=sel_dim):
            shapes = [app.model.get_field_shape(name) for name in field_names if name != 'None']
            sizes = [s.get_size(dim) for s in shapes if dim in s]
    ...
    layout = html.Div(
        style={'width': '100%', 'display': 'inline-block', 'backgroundColor': '#E0E0FF', 'vertical-align': 'middle'},
        children=[
            # --- Settings ---
            html.Div(style={'width': '30%', 'display': 'inline-block', 'vertical-align': 'top'}, children=[
                html.Div(style={'width': '50%', 'display': 'inline-block'}, children=[
                    dcc.Dropdown(options=[{'value': FRONT, 'label': 'Front'}, {'value': RIGHT, 'label': 'Side'},
                                          {'value': TOP, 'label': 'Top'}],
                    ...
                    html.Div(style={'margin-top': 6}, children=[
                        html.Div(style={'width': '90%', 'margin-left': 'auto', 'margin-right': 'auto'}, children=[
                            dcc.Slider(min=0, max=4, step=1, value=0, marks={0: '🡡', 4: '⬤', 1: 'x', 2: 'y', 3: 'z'},
                                       id=f'{viewer_group}_component-slider', updatemode='drag'),
                        ])
                    ]),
                ]),
                html.Div(style={'width': '50%', 'display': 'inline-block', 'vertical-align': 'top'},
                         children=[  # , 'margin-left': 40
                             html.Div('Refresh Rate', style={'text-align': 'center'}),
                             html.Div(
                                 style={'width': '60%', 'margin-left': 'auto', 'margin-right': 'auto', 'height': 50},
                                 children=[
                                     dcc.Slider(min=0, max=3, step=1, value=1, marks={0: 'low', 3: 'high'},
                                                id=REFRESH_RATE.component_id, updatemode='drag'),
                                 ]),
