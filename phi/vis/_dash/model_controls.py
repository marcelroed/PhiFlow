...
def build_model_controls(app: DashApp):
    ...
    layout = html.Div(
        style={'width': '75%', 'margin-left': 'auto', 'margin-right': 'auto', 'background-color': '#F0F0F0'}, children=[
            html.Div(id='control-div',
                     style={'width': '95%', 'height': '90%', 'margin-left': 'auto', 'margin-right': 'auto',
                            'margin-top': 15, 'margin-bottom': 'auto'}, children=[
    ...
    def build_controls(_):
        ...
        for control in model_texts:
            if not control.value_range:
                ...
                dropdown = dcc.Dropdown(id=control.name, options=options, value=control.initial,
...
def create_sliders(controls):
    ...
    for control in controls:
        ...
        if use_log:
            ...
            slider = dcc.Slider(min=slider_min, max=slider_max, value=magn, id=control.name, step=stepsize_magn,
            ...
            slider = dcc.Slider(min=lower, max=upper, value=val, id=control.name, step=step, marks=marks,
                                updatemode='drag')
        slider_container = html.Div(style={'height': '50px', 'width': '100%', 'display': 'inline-block'},
                                    children=[display_name(control.name), slider])
