...
class DashGui(Gui):
    ...
    def setup(self, app: VisModel):
        ...
        @dash_app.dash.callback(Output(REFRESH_INTERVAL.component_id, 'interval'),
                                [REFRESH_RATE, PLAYING, STEP_COMPLETE, REFRESH_INTERVAL])
        ...
        layout = html.Div([
            ...
            html.Div(style={'width': '100%', 'height': 800, 'margin-left': 'auto', 'margin-right': 'auto'},
                     children=[viewers_home[0]]),
            status_bar,
            player_controls,
            model_controls,
            refresh_interval,
        ...
        layout = html.Div([
            ...
            html.Div(style={'width': '50%', 'height': 700, 'display': 'inline-block'},
                     children=[viewers_side_by_side[0]]),
            html.Div(style={'width': '50%', 'height': 700, 'display': 'inline-block'},
                     children=[viewers_side_by_side[1]]),
            status_bar,
            player_controls,
            model_controls,
            refresh_interval,
        ...
        if not caller_is_main and self.config.get('external_web_server', False):
            ...
            if not debug:
                try:
                    ...
                    warnings.warn(
                        'waitress is not installed, falling back to dash development server. To enable it, run  $ pip install waitress',
                        ImportWarning)
            ...
            if use_waitress:
                ...
                self.dash_app.dash.run_server(debug=debug, host=self.config.get('host', '0.0.0.0'), port=port,
                                              use_reloader=False)
            return self
