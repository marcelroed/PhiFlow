...
def box_sdf(x):
    ...
    x = math.rotate_vector(x - center, angle=PI / 4)
...
obstacles = {
    'standard': {
        'box': Obstacle(Box(x=(40, 60), y=(40, 60)), angular_velocity=0.05, mass=1e3),
        # 'circle': Obstacle(Sphere(center=center, radius=radius), angular_velocity=0.05, mass=1e3),
    },
    'levelset': {
        'box': Obstacle(LevelSet(box_sdf, bounds=DOMAIN['bounds'])),
...
normal_vec = math.tensor([math.cos(PI / 6), math.sin(PI / 6)], channel(vector='x,y'))
