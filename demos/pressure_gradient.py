...
OBSTACLE_MASK = HardGeometryMask(OBSTACLES[0].geometry) @ CenteredGrid(0, extrapolation.BOUNDARY, **DOMAIN)
...
pressure = np.zeros((128, 128), dtype=np.float64)
# pressure += velocity.elements.center
pressure_gradient = -np.linspace(0, 1, 128)
pressure[:, :] = pressure_gradient[:, np.newaxis]

pressure = CenteredGrid(math.tensor(pressure, spatial('x,y')), extrapolation.BOUNDARY, **DOMAIN)
